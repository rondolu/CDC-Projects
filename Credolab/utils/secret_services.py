...
class SecretManagerService:
    ...
    def get_credolab_api_key(self) -> str:
        """取得 Credolab API Key（優先從 Secret Manager）。

        優先順序：
        1) config.credolab_secret_version_name → Secret Manager
        2) config.credolab_default_api_key → 預設金鑰（開發用途）
        ...
        """
        version_name = config.credolab_secret_version_name
        if version_name:
            try:
        ...
        # default_key = config.credolab_default_api_key
        # if default_key:
        #     print(default_key)
        #     return default_key
        raise Exception("Credolab API key not configured. Provide secret_version_name or default_api_key in config.")