"""
Credolab Flask 路由藍圖
處理所有與 Credolab 相關的 HTTP 請求和 PubSub 訊息
...
credolab_bp = Blueprint('credolab_bp', __name__)


@credolab_bp.route('/', methods=['POST'])
@Logging.logtobq(task_code="01")
@handle_route_exceptions
def trigger_credolab_flow():
    """
    觸發 Credolab 資料處理流程（帶有批次回調機制）
    Daily Job 觸發點：處理完第一批次後，發布帶有 start_date/end_date 的回調訊息
    """
    # 使用 Service Layer 處理請求
    # Build a shared logger for this request and propagate via context
    shared_log = Logging(mission_name="route_trigger", log_uuid=str(uuid4()), flow_code="E05_credolab")
    ...
    try:
        ...
        _logger.log_text(f"Unhandled exception in trigger_credolab_flow: {str(e)}", severity="Error")
        response_data = {
            "status": "error",
...
@credolab_bp.route('/get_data_range', methods=['POST'])
@Logging.logtobq(task_code="02")
@handle_route_exceptions
def get_data_range():
    """
    ...
    """
    # 使用 Service Layer 處理請求
    shared_log = Logging(mission_name="route_get_data_range", log_uuid=str(uuid4()), flow_code="E05_credolab")
    set_current_log(shared_log)
    service = DataFlowService()
    try:
        data = request.get_json() or {}
